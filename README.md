...
## Why the styling is improved

- Clear visual hierarchy with a centered hero section.
- Compact tables instead of dense paragraphs for page summaries.
- Clean sectioning so the README reads like product documentation, not notes.
- No code changes were made, so functionality is unaffected.
>>>>>>> e2bcd4d (updated documentation)
